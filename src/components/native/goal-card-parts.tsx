  ...
  return (
    <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 16 }}>
      <View style={{ flex: 1, gap: 8 }}>
        ...
        {!!statusLabel && <SmallPill tone={statusLabel === "Выполнена" ? "success" : "danger"}>{statusLabel}</SmallPill>}
      </View>
    </View>
  );
}
...
export function GoalNumericProgressInput({
  progressDelta,
  onChangeProgressDelta,
  onAddProgress,
}: {
  progressDelta: string;
  onChangeProgressDelta: (value: string) => void;
  onAddProgress: () => void;
}) {
  return (
    <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
      ...
      />
      <ActionChip onPress={onAddProgress} tone="primary">
        +
      </ActionChip>
    </View>
  );
