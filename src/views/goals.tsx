...
import {CategoryOption, Goal, GoalFormData, Reminder, Task} from "@/types";
...
export default function Goals() {
  ...
  const handleAddProgress = (id: string, delta: number) => updateGoalProgress(id, delta);
  ...
  function isTaskCompletedThisPeriod(task: Task, goalPeriod: string) {
    ...
    if (goalPeriod === "MONTHLY") return last.getFullYear() === now.getFullYear() && last.getMonth() === now.getMonth();
    return true; // NONE
  };
  ...
  const handleTaskToggle = (goalId: string, taskId: string) => {
    ...
    if (newCompleted) toast.success(`+${task.xpReward ?? 0} XP`);
