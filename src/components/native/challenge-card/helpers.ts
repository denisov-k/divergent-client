...
export function getChallengeDerivedState(challenge: Challenge, userId?: string | null) {
  ...
  const challengeStatus: "COMPLETED" | "FAILED" | "ACTIVE" =
    !isParticipant ? "ACTIVE" : allGoalsCompleted ? "COMPLETED" : challenge.endsAt && new Date(challenge.endsAt) < now ? "FAILED" : "ACTIVE";
